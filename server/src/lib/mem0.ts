...
// Mem0 ka configuration bahut minimal hai — bas ek environment variable. Poori chain ye hai:

// 1. Package install (server/package.json:29)

// "mem0ai": "^3.1.2"
// Ye Mem0 ka official SDK hai. Ye Mem0 cloud (hosted platform) se baat karta hai — koi self-hosted vector DB ya local setup nahi. Pinecone/Postgres ki tarah yahan aapko kuch provision nahi karna, sab unke server pe hota hai.

// 2. Client banta hai — mem0.ts:11-23
// Yahi ekmatra jagah hai jahan Mem0 configure hota hai:
...
//     return client;
// }
// Bas apiKey — koi URL, region, index name, dimensions kuch nahi. Pinecone ke comparison mein dekho (pinecone.ts) — wahan index create karna padta hai, dimensions batane padte hain, ready hone ka wait karna padta hai. Mem0 mein wo sab unki taraf handled hai.

// let client: MemoryClient | null = null upar module-level pe hai — ye singleton pattern hai. Pehli call pe client banta hai, uske baad wahi reuse hota hai. Har request pe naya client nahi banta.

// 3. Env var load kahan hota hai — index.ts:1

// import "dotenv/config";
// Server ki pehli hi line. Ye server/.env file padh ke process.env mein daal deta hai
